...
--the sample data comes from W3schools PostgreSQL tutorial
...
--LIMIT well, limits the max amount of records to return

SELECT * FROM customers
LIMIT 20;

--OFFSET is used to specify where to start counting from 

--Here it starts counting from 41!
SELECT * FROM customers
LIMIT 20 OFFSET 40;
...
--when min or max function is used, by def the returned column is named
--min or max
--to change it, use the AS keyword!

SELECT MIN(price) AS lowest_price
FROM products;

--if you wanna return the record with the min or max price:
...
SELECT COUNT(customer_id) FROM customers AS customer_count;
...
--this is gonna return an ungodly number of decimal places

--to round to x decimal places:
...
--unlike %, _ only represents a single char or number

SELECT * FROM customers
WHERE city LIKE 'L_nd__';

--in is basically syntactic sugar for multiple OR conditions
SELECT * FROM customers
WHERE country IN ('Germany', 'France', 'UK');

--NOT IN will negate the results of IN
SELECT * FROM customers
WHERE country NOT IN  ('Germany', 'France', 'UK');

--this selects all users id appears in orders
--aka every customer that has at least one order!
SELECT * FROM customers
...
--between also works with text
--this selects records that are alphabetically between the two

SELECT * FROM products
WHERE product_name BETWEEN 'Pavlova' AND 'Tofu';

--our data is already sorted, so the results look as expected
--if we didn't group before, use this:
SELECT * FROM products
WHERE product_name BETWEEN 'Pavlova' AND 'Tofu'
ORDER BY product_name;

--BETWEEN also works with dates
SELECT * FROM orders
WHERE order_date BETWEEN '2023-04-12' AND '2023-05-05';

--the AS (Alias) keyboard can be used to concat columns
--use pipe `||` to concat two fields
SELECT product_name || unit AS product
FROM products;

--to add spaces to the concat use || ' ' ||
SELECT product_name || ' ' || unit AS product
FROM products;

--if your alias needs spaces, just use double quotes
SELECT product_name AS "My Great Products"
FROM products;

--let's create some relationships!
--if you check products and categories, you'd see they both have category_id!
